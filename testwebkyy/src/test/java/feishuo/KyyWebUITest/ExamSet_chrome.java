...
public class ExamSet_chrome extends BaseTest{
  ...
  public void testExamSet() throws Exception {
    // 创建考试-正常创建考试
	  ...
	  Reporter.log("勾选全部");//选择班级：class-1是全部
   /*
    //不设时间就默认今天
   ...
   */
   examset.pop_1(driver);//考试要求，常用语1
   
   action.moveToElement(driver.findElement(By.id("SubAudio"))).click().perform();
   Reporter.log("勾选提交录音");
   action.moveToElement(driver.findElement(By.id("showScore2"))).click().perform();
   Reporter.log("勾选全部显示分数");//1是全部不显示分数，2是全部显示分数，3是付费用户显示分数
  ...
  public void testExamNodemend() throws Exception {
    // 创建考试-不填写考试要求
	  Actions action=new Actions(driver);
	  action.moveToElement(driver.findElement(By.id("class-1"))).click().perform();  
	  Reporter.log("选择全部");//选择班级：class-1是全部
   /*
    //不设时间就默认今天
   examset.time(driver, "2017,4,18", "2017,4,19");//考试时间
   examset.ftime_hm(driver, "09","00");//开始时分
   ...
   */
   ...
   action.moveToElement(driver.findElement(By.id("SubAudio"))).click().perform();  
   Reporter.log("勾选提交录音");
   action.moveToElement(driver.findElement(By.id("showScore3"))).click().perform();  
   Reporter.log("勾选全部显示分数");//1是全部不显示分数，2是全部显示分数，3是付费用户显示分数
